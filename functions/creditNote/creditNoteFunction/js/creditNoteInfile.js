...
module.exports.creditNoteInfile = async (event) => {
    try {
...
async function register(params, signInfile) {
    try {
        ...
        console.log(registerInfile); //den
        return Promise.resolve(registerInfile);
    } catch (error) {
        console.log(error); //den
        return Promise.reject(error);
    }
}
...
<dte:Complementos>
<dte:Complemento IDComplemento="` + creditNote.creditNoteComplementId + `" 
NombreComplemento="` + creditNote.creditNoteComplementName + `" 
...
FechaEmisionDocumentoOrigen="` + creditNote.creditNoteSourceDocumentIssuanceDate + `" 
MotivoAjuste="` + creditNote.creditNoteFitMotif + `" 
NumeroAutorizacionDocumentoOrigen="` + creditNote.creditNoteOrigialDocumentAuthorizationNumber + `"  
Version="1">
</cno:ReferenciasNota>
</dte:Complemento>
</dte:Complementos>
...
async function getBodySign(params, xmlBase64) {
    try {
        const jsonInfile = {
            ...
            "archivo": xmlBase64,
            "codigo": params.creditNote.creditNoteCode,
    ...
    try {
        let headers = {
            headers: {
                ...
                'identificador': params.creditNote.creditNoteCode,
            }
        };
        return Promise.resolve(headers);
    } catch (error) {
